...
public class SrvLinkMetaCli extends SrvLinkMeta implements UplinkManager.UplinkOperations {
    ...
    protected void onJSON(byte thrd_ctx, RecycledBuffContext ctx, JSONObject jo) throws Exception {
        ...
        if (JSON_FUNC_welcome.equals(tmp_func)) {
            ...
            if (getLogPrefixName().equals(tmp_db) && username.equals(tmp_login)) { // YYY
                    ...
                    Tum3Logger.DoLog(getLogPrefixName(), true, "Intercon registration failed (mismatching values found: db=" + tmp_db + ", username=" + tmp_login + ")"); // YYY
            }
            if (!WasAuthorized) {
                Tum3Logger.DoLog(getLogPrefixName(), true, "Intercon registration failed.");
                LoginFailedAt = System.currentTimeMillis();
                LoginFailedState = true;
                ShutdownSrvLink("Registration did not match the requested values"); // YYY
            }
            return;
        }
    ...
    private void SendHelloMsg(byte thrd_ctx, RecycledBuffContext ctx) throws Exception {

        JSONObject jo = new JSONObject();
        jo.put(JSON_NAME_function, JSON_FUNC_hello);
        jo.put(JSON_NAME_db, getLogPrefixName());
        jo.put(JSON_NAME_username, username);
        jo.put(JSON_NAME_password, password);
    ...
    public void DoLink() throws Exception {
        ...
        SendHelloMsg(SrvLink.THRD_INTERNAL, null);
